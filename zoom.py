...
def start_listening():
    ...
    t2 = threading.Thread(target=receiver.start_receiver)
    t1.start()
    t2.start()
...
def start_audio_stream():
    auio_sender = AudioSender(text_target_ip.get(1.0, 'end-1c'), 6666)
    t5 = threading.Thread(target=audio_sender.start_stream)
    t5.start()


window = tk.Tk()
window.title("neuraline calls ")
...
btn_listen = tk.Button(window, text="start litening", width=50,command=start_listening)
